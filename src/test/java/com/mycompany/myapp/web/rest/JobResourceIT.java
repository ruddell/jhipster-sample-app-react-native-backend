...
class JobResourceIT {
    ...
    @Test
    @Transactional
    void updateNonExistingJob() throws Exception {
        int databaseSizeBeforeUpdate = jobRepository.findAll().size();
    ...
    void partialUpdateJobWithPatch() throws Exception {
        ...
        restJobMockMvc
            .perform(
                patch("/api/jobs").contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(partialUpdatedJob))
            )
            .andExpect(status().isOk());
    ...
    @Test
    @Transactional
    void partialUpdateJobShouldThrown() throws Exception {
        // Update the job without id should throw
        Job partialUpdatedJob = new Job();
